  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const uploadUrl =
        ...
        `${process.env.NEXT_PUBLIC_BACKEND_URL || DEFAULT_BACKEND_URL}/user/video/upload`;
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        xhr.addEventListener("load", () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            // If the upload fails, provide a more descriptive error message including the status code and status text for debugging
            reject(
              new Error(
                `Upload failed with status ${xhr.status} (${xhr.statusText}). The server may have rejected the file or there may be a network/server issue.`
              )
            );
          }
        });
