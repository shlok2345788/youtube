...
export default function CustomVideoPlayer({
    ...
    // Keep the latest callback without re-attaching video listeners.
    const onLimitReachedRef = useRef(onLimitReached);
    useEffect(() => {
        onLimitReachedRef.current = onLimitReached;
    }, [onLimitReached]);

    const planToMaxWatchSeconds = (plan?: string) => {
        if (plan === "bronze") return 420; // 7 mins
        if (plan === "silver") return 600; // 10 mins
        if (plan === "gold") return Infinity; // unlimited
        return 300; // free/default = 5 mins
    };
    ...
    useEffect(() => {
        ...
        const updateTime = () => {
            ...
            // Calculate time delta for playback
            ...
            // Plan limits (watch time in seconds)
            const maxTime = planToMaxWatchSeconds(user?.plan);

            if (!limitFiredRef.current && accumulatedWatchTimeRef.current >= maxTime) {
                video.pause();
    ...
    return (
        ...
        >
            <video
                ...
                key={video._id}
            >
                <source src={`${video.filepath}#t=0.001`} type="video/mp4" />
