...
const hasSpecialCharacters = (text) => {
  // Allow all letters from any language (\p{L}), numbers (\p{N}), spaces (\s), 
  // and basic punctuation while blocking symbols like @#$%^&*
...
export const getComments = async (req, res) => {
  ...
  try {
    const videoComments = await comments
      .find({
        videoid: videoId,
        isDeleted: false,
      })
...
export const createComment = async (req, res) => {
  ...
  if (hasSpecialCharacters(commentbody)) {
    return res.status(400).json({
      message:
        "Comments with special characters are not allowed. Please use only letters, numbers, and basic punctuation.",
...
export const likeComment = async (req, res) => {
  ...
  try {
    ...
    // Check if dislikes from OTHERS reached 2, auto-delete (unlikely after a like but for consistency)
    const dislikesFromOthers = comment.dislikedBy.filter(id => String(id) !== String(comment.userid)).length;
    if (dislikesFromOthers >= 2 && !comment.isDeleted) {
      comment.isDeleted = true;
      await comment.save();
      return res.status(200).json({
        comment,
        message: "Comment deleted due to excessive dislikes from other users",
      });
    }
...
export const dislikeComment = async (req, res) => {
  ...
  try {
    ...
    // Check if dislikes from OTHERS reached 2, auto-delete
    const dislikesFromOthers = comment.dislikedBy.filter(id => String(id) !== String(comment.userid)).length;
    if (dislikesFromOthers >= 2 && !comment.isDeleted) {
      comment.isDeleted = true;
      await comment.save();
    ...
    res
      .status(500)
      .json({ message: "Error disliking comment", error: err.message });
  }
};
