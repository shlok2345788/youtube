...
const translateText = async (text: string, targetLang: string = "en"): Promise<string> => {
  try {
    ...
    try {
      const altResponse = await fetch("https://translate.mentality.rip/translate", {
        method: "POST",
      ...
      if (altResponse.ok) {
        ...
        return altData.translatedText || text;
      }
    } catch (altError) {
...
// Get user's city - simplified approach to avoid CORS issues
const getUserCity = async (): Promise<string> => {
  try {
    // Simple approach: use timezone to infer general location
    // This avoids CORS issues and doesn't require user permission
    const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;

    // Map common timezones to cities (simplified)
...
// Language detection function (simple heuristic)
const detectLanguage = (text: string): string => {
  ...
  if (cyrillicChars / text.length > 0.1) return 'ru';

  // Could integrate with a language detection service here
...
const Comments = ({ videoId }: CommentsProps) => {
  ...
  const handleTranslate = async (comment: Comment) => {
    console.log("Translate button clicked for comment:", comment._id);
    console.log("Current translatedTexts:", translatedTexts);

    if (translatedTexts[comment._id]) {
      // If already translated, remove translation
      console.log("Removing translation for comment:", comment._id);
    ...
    console.log("Translating comment:", comment._id, "to language:", targetLanguage);
    ...
    try {
      ...
      console.log("Updated translatedTexts:", { ...translatedTexts, [comment._id]: translated });
    } catch (error) {
      console.error("Translation error:", error);
                  ...
                  {translatedTexts[comment._id] ? (
                    <div>
                      <div className="text-xs text-gray-500 mb-1 flex justify-between">
                        <span>Original ({comment.language?.toUpperCase() || '??'})</span>
