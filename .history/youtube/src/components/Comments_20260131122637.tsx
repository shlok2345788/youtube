...
const getUserCity = async (): Promise<string> => {
  try {
    // Try multiple CORS-friendly services
    const services = [
      'https://ipapi.co/json/',
    ...
    // Try ipapi.co first (might work with some browsers)
    try {
      const response = await fetch("https://ipapi.co/json/", {
        mode: 'no-cors'
      });
      // With no-cors mode, we can't read the response, so we'll use a fallback
    } catch (err) {
      // Continue to fallback
    }
    
    // Fallback to a simple approach - use browser's geolocation API
    return new Promise((resolve) => {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          ...
          () => resolve("Unknown"), // User denied location access
          { timeout: 5000 }
        );
      } else {
        resolve("Unknown");
