  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return res.status(200).json({ translatedText: data.translatedText || text });
      }
    } catch (primaryError) {
    ...
    try {
      const fallbackResponse = await fetch("https://translate.mentality.rip/translate", {
        method: "POST",
    ...
    return res.status(500).json({ message: 'Translation service unavailable', translatedText: req.body.text || '' });
  }
}