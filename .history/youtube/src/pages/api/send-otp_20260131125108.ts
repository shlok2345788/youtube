...
const sendSMS = async (
  ...
  if (!process.env.SMS_SERVICE_API_KEY) {
    console.warn(
      "SMS service not configured - OTP will be logged to console only"
    );
    return {
      success: false,
...
const sendEmailOTP = async (email: string, otp: string): Promise<OTPResult> => {
  ...
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    console.warn(
      "Email service not configured - OTP will be logged to console only"
    );
    return {
      success: false,
      error:
        "Email service not configured. Contact admin to set up email credentials.",
