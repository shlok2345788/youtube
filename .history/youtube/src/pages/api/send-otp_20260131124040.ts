...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (isSouthIndian) {
      ...
      if (!email) {
        return res.status(400).json({ message: 'Email is required for South Indian users' });
      }
      result = await sendEmailOTP(email, otp);
    } else {
      // Send OTP via SMS for other users
      if (!phoneNumber) {
        return res.status(400).json({ message: 'Phone number is required for non-South Indian users' });
      }
